...
import { computeScore, type ScoreBreakdown, type ScoreEvent } from "./score";
import { TIMER_DURATION_MIN } from "./progress";
...
export function addScoreEvent(event: ScoreEvent) {
  ...
  const before = computeScore(events, TIMER_DURATION_MIN).total;
  events.push(event);
  writeScoreEvents(events);
  const after = computeScore(events, TIMER_DURATION_MIN).total;
...
export function getScore(): ScoreBreakdown {
  return computeScore(readScoreEvents(), TIMER_DURATION_MIN);
}
